...
# 添加课程屏蔽检查函数
def is_course_blocked(course_name, blocked_courses_list):
    """检查课程是否在屏蔽列表中"""
    if not blocked_courses_list or not course_name:
        return False
    
    course_name = course_name.strip().lower()
    for blocked_course in blocked_courses_list:
        blocked_course = blocked_course.strip().lower()
        if blocked_course and blocked_course in course_name:
            return True
    return False

def get_blocked_courses_list():
    """获取屏蔽课程列表"""
    blocked_courses = config.get('blocked_courses', '').strip()
    if not blocked_courses:
        return []
    
    # 按逗号分割课程名称
    blocked_list = [course.strip() for course in blocked_courses.split(',') if course.strip()]
    if blocked_list:
        print(f"📋 已配置屏蔽课程列表: {', '.join(blocked_list)}")
        print("⚠️ 这些课程将被自动跳过，不会被选择")
    return blocked_list
...
def handle_course_selection(ciframe, course_type="专业课"):
    """处理课程选择的通用函数"""
    # 获取屏蔽课程列表
    blocked_courses_list = get_blocked_courses_list()
                            ...
                            for i in range(numz):
                                try:
                                    ...
                                    try:
                                        ...
                                        # 尝试获取课程名称进行屏蔽检查
                                        course_name_for_check = ""
                                        try:
                                            # 尝试从同一行获取课程名称
                                            row_element = course_links.nth(i).locator("xpath=ancestor::tr")
                                            if row_element.count() > 0:
                                                row_text = row_element.text_content()
                                                # 从行文本中提取课程名称（通常在第一列或第二列）
                                                course_name_for_check = row_text.strip()
                                        except Exception:
                                            # 如果无法获取行信息，使用链接文本作为备选
                                            course_name_for_check = text
                                        
                                        # 检查课程是否被屏蔽
                                        if blocked_courses_list and is_course_blocked(course_name_for_check, blocked_courses_list):
                                            print(f"🚫 跳过屏蔽课程: {course_name_for_check}")
                                            continue
                                        
                                        print(f"准备点击第 {i+1} 个链接: href='{href}', text='{text}'")
                                        if course_name_for_check and course_name_for_check != text:
                                            print(f"课程信息: {course_name_for_check}")
                                    except Exception:
